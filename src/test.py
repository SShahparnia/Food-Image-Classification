...
if __name__ == '__main__':
    ...
    # Replace final layer of resnet50 with a new layer that has the same number of inputs and outputs for our food categories
    model.fc = torch.nn.Linear(num_fc_inputs, num_classes)

    model.load_state_dict(model_dict)

    # Checks if GPU is available for training. If not it defaults to the CPU.
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            test_loss_history.append(batch_loss)
            test_acc_history.append(batch_acc.item())
            print('Batch %d: Test Loss: %.4f, Test Acc: %.4f' % (batch_count, batch_loss, batch_acc))
